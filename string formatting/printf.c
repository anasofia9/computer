...
int vsnprintf(char *buf, int bufsize, const char *format, va_list args)
{
    ...
    for(i =0; i < strlen(format); i++){
        ...
        if(strcmp(curr_format, "%d")==0){
            ...
            characters_added += unsigned_to_base((buf+2 +characters_added), bufsize, num, 16, 0); //doesnt work, min width should be eight but padding does not yet work in my function.
            characters_added +=2;
